...
def random_bipartite_soft_matching(
    ...
    with torch.no_grad():
        B, N, _ = metric.shape
        if use_grid:
            assert ratio == 0.5, "Grid mode only supports ratio 0.5"
            # Handle both even and odd N
            grid_size = N // 2
            sample = torch.randint(2, size=(B, grid_size, 1), device=metric.device)
            sample_alternate = 1 - sample
            grid = torch.arange(0, N - 1, 2).view(1, grid_size, 1).to(device=metric.device)
            grid = grid.repeat(B, 1, 1)  # Repeat for each batch
            rand_idx = torch.cat([sample + grid, sample_alternate + grid], dim=1)
            if N % 2 != 0:
                # Add the last index for odd N
                last_idx = torch.full((B, 1, 1), N - 1, device=metric.device)
                rand_idx = torch.cat([rand_idx, last_idx], dim=1)
        else:
